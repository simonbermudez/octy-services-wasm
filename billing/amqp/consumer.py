...
def handle_message(payload, main_loop) -> None:
    ...
    try:
        if routing_key == 'account.billing.cmd.capture':
            loop.run_until_complete(BillingService.calculate_persist_billable_units(u.units))
    except Exception as ex:
        logger.error(f'Error calculating and persisting billable units: {ex}')
