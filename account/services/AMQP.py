        ...
        async with message_payload.process(ignore_processed=True):
            ...
            try:
                ...
                if routing_key == 'account.configs.cmd.update':
                    await accountRepository.update_account(account, 'account-config')
                elif routing_key == 'algo.configs.cmd.update':
                    await accountRepository.update_account(account, 'algorithm-config')
                elif routing_key == 'churn.info.cmd.update':
                    await accountRepository.update_account(account, 'churn-info')
            except Exception as e:
                print(e)
                await message_payload.reject() 
            else:
                await message_payload.ack()
...
class AMQPPublisher():
    """
        ...
        ----------
        ...
        # NOTE: This class can only handle opening a channel for a single publisher per service.
