# module imports
from data.models.recommendations import DeleteRecCache
...
def ack_message(payload, did_succeed : bool=True, requeue : bool=True) -> None:
    try:
        ...
        logger.info(f"Acknowledged message! Requeued message: {requeue}")
...
def handle_message(payload, main_loop) -> None:
    ...
    try:
        ...
        job_data = DeleteRecCache(**message_json)
    ...
    try:
        if routing_key == 'reccache.cmd.delete':    
            loop.run_until_complete(recommendationsRepository.\
                delete_cached_recommendations(account_id=job_data.account_id, profiles=job_data.profiles))
    except Exception as ex:
        logger.error(f'Error deleting cached recommendations: {ex}')
