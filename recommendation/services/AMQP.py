    ...
    async def message_handler_callback(self, message_payload : object):
        ...
        async with message_payload.process(ignore_processed=True):
            ...
            if routing_key == 'reccache.cmd.delete':
                ...
                await recommendationsRepository.delete_cached_recommendations(account_id=job_data.account_id, profiles=job_data.profiles)
...
class AMQPPublisher():
    """
        ...
        Handles:
        - Amqp (RabbitMQ) publisher instances
        ...
...
class AMQPStateManager():
    """
        AMQPStateManager
        Handles:
        - Initialising AMQP desired connection(s) state
        - Persisting AMQP desired connection(s) state
        ...
